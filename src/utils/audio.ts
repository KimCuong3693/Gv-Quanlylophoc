// Simple Web Audio API sound synthesizer for classroom interactions
...
export function playPointDeductSound() {
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    const now = ctx.currentTime;
...
export function playBloomCelebrationSound() {
  try {
    const ctx = getAudioContext();
    if (!ctx) return;
    const now = ctx.currentTime;
...
export function playTimerFinishSound() {
  try {
    ...
    [0, 0.25, 0.5].forEach((offset) => {
      ...
      osc.type = 'square';
      osc.frequency.setValueAtTime(880, noteTime); // A5
